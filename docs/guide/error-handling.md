...
If a registered handler throws, Hibiki responds with `500` and body `HIBIKI_HANDLER_FAILED`. Unexpected non-`HibikiError` failures become a generic `500 Internal webhook error` without leaking internals.
